{
  ...
  const $drawText = (x, y, text, options = {}) => {
    ...
    if (angle) {
      ...
      return withAngle(x, y, angle, () => $drawText(0, 0, text, options));
    }
  ...
  const $drawRect = (x, y, w, h, options = {}) => {
    ...
    if (angle) {
      ...
      return withAngle(x, y, angle, () => $drawRect(0, 0, w, h, options));
    }
  ...
  const $drawLine = (fromX, fromY, toX, toY, { color, lineWidth }) => {
  ...
  const $drawCircle = (
    ...
    y,
    r,
    { color, fill, startAngle = 0, endAngle = tPI, aCW = true, lineWidth = 1.5 }
  ) => {
    ctx.lineWidth = lineWidth;
    ctx.strokeStyle = color;
    ctx.fillStyle = color;
  ...
  const draw = once => {
    ...
    const quadrant = { '01': 1, '00': 2, '10': 3, '11': 4 }[
      `${+(sin > 0)}${+(cos > 0)}`
    ];
    ...
    // Draw theta name.
    config.drawNameTheta &&
      $drawText(x + 20, y - 15, `θ`, {
        color: COLORS.greenDark,
        align: 'left'
      });
    ...
    const rightAngleSquareHeight =
      quadrant > 2 ? -rightAngleSquareSide : rightAngleSquareSide;
    $drawRect(lineX, lineY, rightAngleSquareSide, rightAngleSquareHeight, {
      color: COLORS.gray,
      angle: tanCotAngle,
      lineWidth: 1
    });
    ...
    if (degree % 90) {
      ...
      // Draw radius text.
      config.drawNameRadius &&
        $drawText(x + (lineX - x) / 2, y + (lineY - y) / 2, name.r, {
          color: COLORS.night,
          angle: -degree
        });
      // Draw sinus text.
      config.drawNameSin &&
        $drawText(lineX + 1, y + (lineY - y) / 2, name.sin, {
          color: COLORS.purple,
          angle: 90
        });
      // Draw cosinus text.
      config.drawNameCos &&
        $drawText(x + (lineX - x) / 2, lineY - 2, name.cos, {
          color: COLORS.green
        });
      // Draw tangent text.
      config.drawNameTan &&
        $drawText(
          ...
          y - 5 + (lineY - y) / 2,
          name.tan,
          { color: COLORS.orange, angle: tanCotAngle }
        );
      // Draw cotangent text.
      config.drawNameCot &&
        $drawText(
          x + (lineX - x) / 2,
          lineY - 5 + (cotY - lineY) / 2,
          name.cot,
          { color: COLORS.pink, angle: tanCotAngle }
        );
      // Draw secant text.
      config.drawNameSec &&
        $drawText(x + (tanX - x) / 2, y + 17, name.sec, { color: COLORS.blue });
      // Draw cosecant text.
      config.drawNameCsc &&
        $drawText(x - 17, y + (cotY - y) / 2, name.csc, {
          color: COLORS.cyan,
          angle: 90
        });
    }
    ...
    config.drawNameFPS &&
      config.play &&
      !$state.drag &&
      $drawText(5, 17, `FPS: ${calculateFPS() || '-'}`, { align: 'left' });

    // Draw name of author and help.
    config.drawNameInfo &&
      $drawText(w - 10, 20, 'You can click & drag!', { align: 'right' });
  ...
  canvas.addEventListener('mousedown', event => {
    $state.drag = true;
    setDegreeByClientPosition(event.offsetX, event.offsetY);
  });

  canvas.addEventListener('mousemove', event => {
    $state.drag && setDegreeByClientPosition(event.offsetX, event.offsetY);
  });
