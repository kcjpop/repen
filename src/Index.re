...
[@bs.val] [@bs.scope "document"] external getCanvasById: string => canvas = "getElementById";
...
let draw = (canvas: canvas, ctx: context) => {
  ...
  /* Draw X axis */
  drawLine(~fromX=0., ~fromY=y, ~toX=float_of_int(w), ~toY=y, ~color=Colors.gray, ctx);

  /* Draw Y axis */
  drawLine(~fromX=x, ~fromY=0., ~toX=x, ~toY=float_of_int(h), ~color=Colors.gray, ctx);
  ...
  let coDegree = isEvenQuad ? 90. -. mod_float(deg, 90.) : mod_float(deg, 90.);
  ...
  /* Draw theta */
  drawCircle(~x, ~y, ~r=20., ~color=Colors.greenLight, ~shouldFill=true, ~endAngle=degToRad(360. -. deg), ctx);
  drawCircle(~x, ~y, ~r=20., ~color=Colors.greenDark, ~shouldFill=false, ~endAngle=degToRad(360. -. deg), ctx);
  drawText(
    ~x=x +. labelPadding *. 5.,
    ~y=y -. labelPadding *. 2.,
    ~text={js|θ|js},
  ...
  /* Draw radius */
  drawLine(~fromX=x, ~fromY=y, ~toX=lineX, ~toY=lineY, ~color=Colors.night, ctx);
  drawText(
    ~x=x +. (lineX -. x) /. 2.,
  ...
  /* Draw sin */
  drawLine(~fromX=lineX, ~fromY=lineY, ~toX=lineX, ~toY=y, ~color=Colors.purple, ctx);
  drawText(~x=lineX +. labelPadding, ~y=y +. (lineY -. y) /. 2., ~text="sin", ~color=Colors.purple, ~angle=90., ctx);

  /* Draw cos */
  drawLine(~fromX=lineX, ~fromY=lineY, ~toX=x, ~toY=lineY, ~color=Colors.green, ctx);
  drawText(~x=x +. (lineX -. x) /. 2., ~y=lineY -. labelPadding, ~text="cos", ~color=Colors.green, ctx);

  /* Draw tan */
  drawLine(~fromX=tanX, ~fromY=y, ~toX=lineX, ~toY=lineY, ~color=Colors.orange, ctx);
  drawText(
  ...
  /* Draw cotan */
  drawLine(~fromX=lineX, ~fromY=lineY, ~toX=x, ~toY=cotY, ~color=Colors.pink, ctx);
  drawText(
    ~x=x +. (lineX -. x) /. 2.,
  ...
  drawText(~x=x +. (tanX -. x) /. 2., ~y=y +. labelPadding *. 3., ~text="secant", ~color=Colors.blue, ctx);
  ...
  drawCircle(~x=lineX, ~y=lineY, ~r=3., ~color=Colors.night, ~shouldFill=true, ctx);
};
